...
while True:
    roi, frame = video.read() 
    videoEn = video.shape[1] #görüntünün genişliğini (sütun sayısı) alır 
    videoBoy = video.shape[0] #görüntünün yüksekliğini (satır sayısı) alır
    videoBlob = cv2.dnn.blobFromImage(video, 1/256, (416,416), swapRB= True, crop=False)
    #bilgisayarın anlayacağı videoğraf formatına çevirir
    ...
    model.setInput(videoBlob)  # videoğrafı model ile eşleştir
    ...
    for katmanlar in bulunanKatmanlar:
        for nesne in katmanlar:
            ...
            if(guvenpuani > 0.50):    
                ...
                dortgen = nesne[0:4] * np.array([videoEn, videoBoy,videoEn, videoBoy])
    ...
    for maxID in maxGuvenirlikID:
        ...
        cv2.rectangle(video, (startX, startY), (endX, endY), dortgenRengi, 1)
        cv2.putText(video, yuzde, (startX, startY-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, dortgenRengi, 2)


    cv2.imshow("frameğraf yolu", video)
